#!/usr/bin/env python3
"""
NFL Next Gen Stats Scraper with Auto Header Fix

This script scrapes NFL Next Gen Stats and automatically fixes CSV headers
if they are numeric (0,1,2,3...) by replacing them with proper stat names.

Usage:
    python tests/nfl_stats_scraper.py
    python tests/nfl_stats_scraper.py --year 2025 --week 2
"""
...
def ngs_passing_url(year=2025, phase="REG", week=1, sort_hash="yards"):
    ...
    return f"https://nextgenstats.nfl.com/stats/passing/{year}/{phase}/{week}#{sort_hash}"
...
async def fetch_ngs_passing_table(year=2025, week=1, phase="REG", sort_hash="yards"):
    """Fetch NFL Next Gen Stats passing table"""
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Scrape NFL Next Gen Stats with header fixing')
    ...
    if args.output:
        ...
        output_file = script_dir / f"ngs_passing_{args.year}_wk{args.week}_{args.sort}.csv"
    
    headers_file = Path(__file__).parent / "passing_headers.csv"
    
    print("=" * 80)
    print("NFL NEXT GEN STATS SCRAPER")
    print("=" * 80)
    ...
    try:
        # Scrape the data
        print(f"\n🏈 Scraping NFL Next Gen Stats for {args.year} Week {args.week}...")
        df = asyncio.run(fetch_ngs_passing_table(
            year=args.year, 
            week=args.week, 
            phase=args.phase, 
            sort_hash=args.sort
        ...
        print(f"\n✅ NFL Stats scraping completed successfully!")
